<?php get_header() ?>

<?php include_once('page-pesquisa.php');?>

<div class="container">
	<div class="titleLoja">
		<h3><?php the_title();?></h3>
		<!-- Titulo Loja -->
		<?php $categoria = get_terms('categoria'); ?>
		<?php $piso = get_terms('piso'); ?>
		<p><?php foreach ( $categoria as $categoriaInfo ) {
					echo strtoupper($categoriaInfo->name);
				}
 ?> - <?php foreach ( $piso as $pisoInfo ) {
    echo strtoupper($pisoInfo->name);
} ?></p>
		<!-- Categoria - Piso Loja -->
	</div>		
	<div class="row">
		<div class="col-sm-12 col-lg-6 infosLoja">
			<div class="imgLoja">
				<img src="<?php the_field('field_5c6ed966788cb');?>" alt="loja">
			</div>
			<div class="descLoja">
				<p><?php the_content();?></p>
				<!-- Descrição Loja -->
			</div>
			
			<div class="row">
				<div class="col-sm-12 col-md-6">
					<h5 class="numero"><?php the_field('field_5c6ed74f788c6');?></h5>
					<!-- Numero Loja -->
				</div>
				<div class="col-sm-12 col-md-6">
					<h5 class="site"><?php the_field('field_5c6ed825788c8');?></h5>
					<!-- Site Loja -->
				</div>
			</div>
			<div class="row">
				<div class="col-sm-12 col-md-6">
					<h5 class="piso"><?php foreach (wp_get_object_terms(get_the_ID(),'piso') as $category){
					 echo $category->name;
				 }?></h5>
					<!-- Piso Loja -->
				</div>
				<div class="col-sm-12 col-md-6">
					<h5 class="numeroLoja">nº <?php the_field('field_5c6ed874788c9');?></h5>
					<!-- Numero Loja -->
				</div>
			</div>
			<div class="row">
				<div class="col-sm-12 col-md-6">
					<h5 class="horario"><?php the_field('field_5c6ed7a5788c7');?></h5>
					<!-- Horario Loja -->
				</div>
				<div class="col-sm-12 col-md-6">
					<h5 class="categoria"><?php the_field('field_5ca4b2d00d32e');?></h5>
					<!-- Categoria Loja -->
				</div>
			</div>


		</div>
		<div class="col-sm-12 col-lg-6">
			<div class="mapaLoja">
				<?php $numeroLoja = get_field('field_5c6ed874788c9');
    ?><img src="<?php echo get_template_directory_uri(); ?>/img/mapaLoja/Loja<?php echo $numeroLoja ?>.png">
			</div>
		</div>
	</div>
</div>

<?php include_once('page-newsletter.php');?>











<?php get_footer() ?>